...
def capture_screenshot():
    """Capture une capture d'écran de l'application ERREUR 404"""
    pygame.init()
    
    # Configuration
    config = Config()
    
    # Initialisation de l'écran en mode fenêtré pour la capture
    screen = pygame.display.set_mode((1920, 1080))
    pygame.display.set_caption("ERREUR 404 - Capture d'écran")
    
    # Composants
    matrix_rain = MatrixRain(screen, config)
    glitch_text = GlitchText(config)
    ...
    matrix_rain.update()
    glitch_text.draw(screen, width // 2, height // 2)
    pygame.display.flip()
    
    # Attendre un peu pour que les effets se stabilisent
    time.sleep(0.5)
    
    # Capturer l'écran
    screenshot = pygame.surface.Surface((width, height))
    screenshot.blit(screen, (0, 0))
    
    # Sauvegarder l'image
    pygame.image.save(screenshot, "screenshot.png")
    print("Capture d'écran sauvegardée dans 'screenshot.png'")
